package main

import (
	"fmt"

	dbexporter "github.com/prajakta-scalent/rule-engine/pkg/db-exporter"
	ruleengine "github.com/prajakta-scalent/rule-engine/pkg/rule-engine"
	ruleimporter "github.com/prajakta-scalent/rule-engine/pkg/rule-importer"
)

func main() {
	// user := services.User{
	// 	Id:  1,
	// 	Age: 18,
	// }

	// rules := []ruleengine.Rule{
	// 	{
	// 		Name:        "AgeShouldBeMoreThan",
	// 		Condition:   "gte",
	// 		MatchValue:  18,
	// 		IsMandatory: false,
	// 	},
	// 	{
	// 		Name:        "NameEqualTo",
	// 		Condition:   "eq",
	// 		MatchValue:  "john",
	// 		IsMandatory: true,
	// 	},
	// 	{
	// 		Name:        "BalanceMoreThan",
	// 		Condition:   "gte",
	// 		MatchValue:  0.0,
	// 		IsMandatory: true,
	// 	},
	// }

	// rules1 := []ruleengine.Rule{
	// 	{
	// 		Name:        "AgeShouldBeMoreThan",
	// 		Condition:   "gte",
	// 		MatchValue:  30,
	// 		IsMandatory: true,
	// 	},
	// 	{
	// 		Name:        "APICallCheckAgeAllowed",
	// 		Condition:   "callback",
	// 		IsMandatory: true,
	// 	},
	// }

	// ruleGroup := ruleengine.RuleGroup{
	// 	Name:                "userRulesGroup",
	// 	Rules:               rules,
	// 	ExecuteConcurrently: true,
	// }

	// ruleGroup2 := ruleengine.RuleGroup{
	// 	Name:                "userTestRulesGroup",
	// 	Rules:               rules1,
	// 	ExecuteConcurrently: true,
	// }

	ruleInput := map[string][]ruleengine.Input{
		"userRulesGroup": {
			{
				RuleName: "AgeShouldBeMoreThan",
				Value:    20,
			},
			{
				RuleName: "NameEqualTo",
				Value:    "john",
			},
			{
				RuleName: "BalanceMoreThan",
				Value:    0.00,
			},
		},
	}

	ruleImporter := ruleimporter.RuleImporter{}
	dbExporter := dbexporter.DBExporter{}

	ruleEngine, err := ruleengine.New(dbExporter, ruleImporter)
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	rules, err := ruleEngine.RegisterRule("pkg/rule-importer/ruledata.json")
	if err != nil {
		fmt.Println(err.Error())
		return
	}
	ruleEngine.RegisterGroup("userRulesGroup", rules, true)
	executionID, result, err := ruleEngine.Execute(ruleInput)
	fmt.Println("\n\nFinal Result: \n", executionID, result, err)
}
